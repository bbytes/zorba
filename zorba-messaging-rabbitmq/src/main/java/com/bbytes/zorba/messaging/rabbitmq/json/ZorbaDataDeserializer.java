...
public class ZorbaDataDeserializer extends JsonDeserializer<ZorbaData<String, Serializable>> {
	...
	private static final Logger log = Logger.getLogger(ZorbaDataDeserializer.class);
	...
	public ZorbaData<String, Serializable> deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException,
		...
		for (; t != JsonToken.END_OBJECT; t = jp.nextToken()) {
			...
			case START_ARRAY:
				...
				zorbaData.put(key, (Serializable) arrayObj);
				break;
			case START_OBJECT:
	...
	private Object deserializeFromCustomObject(JsonParser jp, DeserializationContext ctxt) throws JsonParseException,
		 ...
		 * clazz = Class.forName(className); return zorbaObjectMapper.readValue(jp,
		 * CustomAddress.class);
		 */
